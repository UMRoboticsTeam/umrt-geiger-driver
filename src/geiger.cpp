...
Geiger::Geiger(const std::string &addr) {
  ...
  if (fd == -1) {
    BOOST_LOG_TRIVIAL(error)
        << "[x] could not open serial file: " << addr << " errno: " << errno;
  ...
  if (tcgetattr(fd, &termios_struct) == -1) {
    ...
    fd = -1;
    BOOST_LOG_TRIVIAL(error)
        << "[x] could not get current termios config: " << addr
        << " errno: " << errno;
    throw runtime_error("could not get current termios config");
  }
  // apply custom termios config
  termios_struct.c_cflag &=
      ~(CSTOPB | PARENB | CSIZE); // 1 stop bit and no parity bit
  termios_struct.c_cflag |=
      (CS8 | CREAD |
       CLOCAL); // 8-bit data width, enable receiver, ignore modem controls.
  cfsetospeed(&termios_struct,
              B1152000); // set fixed output baud rate to 1152000 per
                         // competition specifications
  cfsetispeed(&termios_struct,
              B1152000); // set fixed input baud rate to 1152000 per competition
                         // specifications
  termios_struct.c_cc[VMIN] = 1;
  termios_struct.c_cc[VTIME] = 0;

  if (tcsetattr(fd, TCSANOW, &termios_struct) ==
      -1) { // set configuration immediately
    close(fd);
    fd = -1;
    BOOST_LOG_TRIVIAL(error)
        << "[x] could not set termios config. errno: " << errno;
    throw runtime_error("could not get current termios config");
  }
